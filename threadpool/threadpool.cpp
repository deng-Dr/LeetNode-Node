...
TaskQueue::TaskQueue(){
    pthread_mutex_init(&m_mutex, NULL);         //互斥锁的初始化
}

TaskQueue::~TaskQueue(){
    pthread_mutex_destroy(&m_mutex);           //消除互斥锁
}
//  队列中的插入和删除操作都需要上锁
...
private:
    ...
    int m_maxNum;                   //线程最大数量
    int m_busyNum;                  //忙线程数量
    int m_aliveNum;                 //存货线程数量
    int m_exitNum;                  
    bool m_shutdown = false;        //标志线程池是否宕机，线程池在添加任务的时候就需要先判断是否宕机
};
